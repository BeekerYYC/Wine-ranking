...
/** The wine's own photo, if it has one — never the AI-found label image. */
export function winePhotoSrc(wine: WineImageRef): string | null {
  if (wine.imageData) return wine.imageData;
  if (wine.hasImage && wine.id != null) return `/api/wines/${wine.id}/image`;
  return null;
}

export function wineImageSrc(wine: WineImageRef): string | null {
  return winePhotoSrc(wine) || wine.labelImageUrl || null;
}
