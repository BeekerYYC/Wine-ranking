...
export default function EnrichAllModal({
  category,
  onClose,
  onComplete,
}: {
  ...
  useEffect(() => {
    fetch(`/api/wines/missing-enrichment?category=${category}&status=collection`)
      .then((r) => r.json())
      .then((data) => {
        setWines(data.wines || []);
        setPhase("ready");
  ...
  const handleRun = async () => {
    ...
    // Sequential — each enrich call can take 15-30s (Claude + label search),
    // and running them in parallel risks hitting Anthropic rate limits.
    for (let i = 0; i < wines.length; i++) {
      const w = wines[i];
      setProgress({ done: i, ok, failed, current: w.name });
      try {
        const res = await fetch(`/api/wines/${w.id}/enrich`, { method: "POST" });
        if (res.ok) {
          ok++;
        } else {
          failed++;
    ...
    setProgress({ done: wines.length, ok, failed, current: "Done" });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-bg/80 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <div className="p-5 border-b border-border-subtle flex items-start justify-between gap-3">
          <div>
            <h2 className="text-[16px] font-semibold text-text-primary">Enrich all wines</h2>
            <p className="text-[12px] text-text-tertiary mt-0.5">
              Run AI enrichment on every wine that's missing tasting notes, critic reviews, etc.
            </p>
          </div>
        ...
        <div className="p-5">
          ...
          {phase === "error" && <p className="text-[13px] text-danger">{errorMessage}</p>}
          ...
          {phase === "ready" && wines.length === 0 && (
            <div className="text-center py-4">
              ...
              <p className="text-[14px] font-semibold text-text-primary">All wines are enriched</p>
              <p className="text-[12px] text-text-tertiary mt-0.5">Nothing to do.</p>
            </div>
          )}
          ...
          {phase === "ready" && wines.length > 0 && (
            <>
              <p className="text-[13px] text-text-secondary mb-3">
                <span className="text-gold font-semibold">{wines.length}</span> wine{wines.length !== 1 ? "s" : ""} need enrichment.
              </p>
              ...
              <p className="text-[11px] text-text-muted mb-3">
                Each takes ~15–30s. Roughly {Math.ceil((wines.length * 20) / 60)} minute{Math.ceil((wines.length * 20) / 60) !== 1 ? "s" : ""} total. Keep this tab open.
              </p>
              <button
                onClick={handleRun}
                className="w-full bg-gold/90 hover:bg-gold text-bg py-2.5 rounded-lg text-[13px] font-semibold transition-colors"
              >
                Enrich {wines.length} wine{wines.length !== 1 ? "s" : ""}
              </button>
            </>
          )}
          ...
          {phase === "running" && (
            <>
              ...
              <p className="text-[11px] text-text-muted tabular-nums mb-3">
                {progress.done} of {wines.length} · {progress.ok} ok · {progress.failed} failed
              </p>
              <p className="text-[13px] text-text-secondary truncate">
                <span className="text-text-muted">Enriching:</span> {progress.current}
              </p>
            </>
          )}
          ...
          {phase === "done" && (
            <>
              ...
              <p className="text-center text-[14px] font-semibold text-text-primary">
                Enriched <span className="text-gold">{progress.ok}</span> of {wines.length}
              </p>
              {progress.failed > 0 && (
                <div className="mt-3 bg-danger-muted/40 border border-danger/15 rounded-lg p-3">
                  <p className="text-[11px] text-danger font-medium mb-1">{progress.failed} failed (you can re-run later):</p>
